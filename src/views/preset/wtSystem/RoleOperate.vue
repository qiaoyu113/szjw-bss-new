<template>
  <div :class="isPC ? 'RoleOperate' : 'RoleOperate-m'">
    <el-form
      ref="ruleForm"
      :model="ruleForm"
      :rules="rules"
...
export default class extends Vue {
  ...
  private sysType:number | null = null
  private id: any = '';
  ...
  private async getAuth() {
    let params = { sysType: this.sysType }
    const { data } = await authorityList(params)
    if (data.success) {
  ...
  mounted() {
    ...
    this.sysType = +this.$route.query.sysType
    this.isEdit = this.$route.name === 'RoleOperate'
    this.fetchData()
  }
}
</script>
